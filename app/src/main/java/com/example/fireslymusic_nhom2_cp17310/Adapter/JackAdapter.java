...
public class JackAdapter extends RecyclerView.Adapter<JackAdapter.JackViewHolder>{
    List<Casi> listSong;
    Context context;
    List<Casi> listv1 = new ArrayList<>();
    public JackAdapter(Context context) {
    ...
    @NonNull
    @Override
    public JackAdapter.JackViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_chitietcasi,parent,false);
        return new JackViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull JackAdapter.JackViewHolder holder, @SuppressLint("RecyclerView") int position) {
    ...
    public class JackViewHolder extends RecyclerView.ViewHolder {
        ImageView img_song;
        TextView tv_namesong,tv_casi;
        LinearLayout layout_bhtd;


        public JackViewHolder(@NonNull View itemView) {
            super(itemView);
