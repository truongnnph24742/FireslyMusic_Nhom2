...
public class USUKAdapter extends RecyclerView.Adapter<USUKAdapter.USUkHolder> {
    List<Category> listSong;
    Context context;
    List<Category> listv1 = new ArrayList<>();
    public USUKAdapter(Context context) {
    ...
    @NonNull
    @Override
    public USUKAdapter.USUkHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_baoanh,parent,false);
        return new USUkHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull USUKAdapter.USUkHolder holder, @SuppressLint("RecyclerView") int position) {
    ...
    public class USUkHolder extends RecyclerView.ViewHolder {
        ImageView img_song;
        TextView tv_namesong,tv_casi;
        LinearLayout layout_casi;


        public USUkHolder(@NonNull View itemView) {
            super(itemView);
