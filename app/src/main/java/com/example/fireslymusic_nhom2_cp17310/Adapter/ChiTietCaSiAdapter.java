...
public class ChiTietCaSiAdapter extends RecyclerView.Adapter<ChiTietCaSiAdapter.ChiTietCaSiViewHolder> {
    List<Casi> listSong;
    Context context;
    List<Casi> listv1 = new ArrayList<>();
    public ChiTietCaSiAdapter(Context context) {
    ...
    @NonNull
    @Override
    public ChiTietCaSiViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_chitietcasi,parent,false);
        return new ChiTietCaSiViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull ChiTietCaSiViewHolder holder,@SuppressLint("RecyclerView") int position) {
    ...
    public class ChiTietCaSiViewHolder extends RecyclerView.ViewHolder {
        ImageView img_song;
        TextView tv_namesong,tv_casi;
        LinearLayout layout_bhtd;


        public ChiTietCaSiViewHolder(@NonNull View itemView) {
            super(itemView);
