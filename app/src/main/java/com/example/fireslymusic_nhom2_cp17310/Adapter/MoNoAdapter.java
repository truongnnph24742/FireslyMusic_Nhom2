...
public class MoNoAdapter extends RecyclerView.Adapter<MoNoAdapter.MoNoViewHolder> {
    List<Casi> listSong;
    Context context;
    ...
    @NonNull
    @Override
    public MoNoAdapter.MoNoViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_chitietcasi,parent,false);
        return new MoNoViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull MoNoAdapter.MoNoViewHolder holder, @SuppressLint("RecyclerView") int position) {
    ...
    public class MoNoViewHolder extends RecyclerView.ViewHolder {
        ImageView img_song;
        TextView tv_namesong,tv_casi;
        LinearLayout layout_bhtd;


        public MoNoViewHolder(@NonNull View itemView) {
            super(itemView);
